...
patternMatching returns [PatternDeclaration patternRet]://TODO:cunstruct patterDeclaration node
    ...
    {
        Identifier tar_ = new Identifier($targetVar.text);
        tar_.setLine($patternName.line);
        Identifier name_ = new Identifier($patternName.text);
        name_.setLine($targetVar.line);
        $patternRet = new PatternDeclaration(name_, tar_);
        $patternRet.setLine($targetVar.line);
        ArrayList<Expression> cRet = new ArrayList<Expression>();
    
    }

    RPAR
    (PATTERN_MATCHING_SEPARATOR c = condition{cRet.addAll($c.conditionRet);}
     ASSIGN e = expression{$patternRet.addReturnExp($e.expRet);}
     )*
    {$patternRet.setConditions(cRet);}
    SEMICOLLON;
...
pushStatement returns [PushStatement pushRet]://TODO:construct pushStatement node
    p = PUSH LPAR e1 = expression COMMA e2 = expression
    {
...
range returns [ArrayList<Expression> rangeRet]://TODO:store all expressions that a range can have in rangeRet array
    ...
    (LPAR e1 = expression{$rangeRet.add($e1.expRet);}
    DOUBLEDOT e2 = expression{$rangeRet.add($e2.expRet);}
    RPAR)
    |
...
matchPatternStatement returns [MatchPatternStatement matchPatRet]://TODO:construct match pattern node
    id = IDENTIFIER DOT m = MATCH LPAR e = expression
    {
        Identifier id_ = new Identifier($id.text);
        id_.setLine($id.line);
...
expression returns [Expression expRet]:
    ...
    a = APPEND{line = $a.line;} 
    e2 = eqaulityExpression
    {

        if ($e1.expRet instanceof AppendExpression A){
            A.addAppendedExpression($e2.expRet);
            $expRet = A;
        }
        else{
             AppendExpression temp = new AppendExpression($e1.expRet);
             temp.addAppendedExpression($e2.expRet);
             $expRet = temp;
        }
        $expRet.setLine(line);
